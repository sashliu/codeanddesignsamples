...
class OneHotEncoder(Callable):
    ...
    def fit(self, data):
        ...
        self.uniq = set(data)  # all the unique labels from `data`
        onehot = np.eye(len(self.uniq))
        self.uniq2oh = dict(zip(self.uniq, onehot)) #{e : onehot[i] for i, e in enumerate(self.uniq)}   # a lookup dictionary with labels and corresponding encodings
