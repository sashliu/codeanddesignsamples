...
class SequentialModel(Beras.Model):
    ...
    def batch_step(self, x, y, training=True):
        ...
        acc = self.compiled_acc(y_pred, y)
...
def get_advanced_model_components():
    ...
    model = SequentialModel([Dense(784, 100), LeakyRelU()],[Dense(100, 10), Softmax()])
    model.compile(
        optimizer=Adam(0.02),
        loss_fn=MeanSquaredError(),
        acc_fn=CategoricalAccuracy(),
...
if __name__ == "__main__":
    ...
    ohe = lambda x: OneHotEncoder.fit(x) ## placeholder function: returns zero for a given input
