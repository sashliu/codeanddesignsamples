...
def sk_learn_cluster(X, K):
    ...
    return(kmean.cluster_centers_, kmean.predict(X))
...
def cluster_songs(music_data, max_iters=300):
    ...
    new_data = min_max_scale(music_data)
    # TODO: Visualize the raw data with visualize_songs_clusters
    visualize_songs_clusters(new_data)
    # TODO: Use elbow_point_plot() to determine the optimal K (number of clusters)
    for i in range(1, MAX_CLUSTERS + 1):
        kmean = Kmeans(new_data, i, max_iters)
        test_centroids, test_indices = kmean.run()
        clusters.append(i)
        inertias.append(kmean.inertia(test_centroids, test_indices))

    print(clusters)
    print(inertias)
    elbow_point_plot(np.array(clusters), np.array(inertias))
    # TODO: Run k-means with your chosen K
    set_kmean = Kmeans(new_data, 6, max_iters)
    centroids, idx = set_kmean.run()
    # TODO: Use the visualize_songs_clusters() function to plot clusters
    centroids_sklearn, idx_sklearn = sk_learn_cluster(new_data, 4)
    visualize_songs_clusters(new_data, centroids=centroids, centroid_indices=idx)
    # TODO: Visually compare the output from your Kmeans object to sk_learn_cluster() (print clusters and plot the song clusters)
    visualize_songs_clusters(new_data, centroids= centroids_sklearn, centroid_indices=idx_sklearn, is_lib_kmean=True)
